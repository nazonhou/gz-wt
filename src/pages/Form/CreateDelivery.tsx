...
const CreateDelivery = () => {
  ...
  const {
    errorMessage: getPackagesErrorMessage,
  ...
  useEffect(() => {
    getPackages({ url: '/package' });
  }, []);

  return (
    <DefaultLayout>
      <Breadcrumb pageName="Create Delivery" />
      ...
      <Formik
        ...
        validationSchema={Yup.object({
          package_id: Yup.string().required('Required').uuid('Is not valid'),
              ...
              <div className="flex flex-col gap-9">
                <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                  <div className="p-6.5">
                    ...
                    <div className="mb-4.5">
                      <Select
                        errors={validationErrors['package_id'] ?? []}
                        label="Package"
                        name="package_id"
                      >
                        <option
                        ...
                        >
                          Select the package to link
                        </option>
                        {getPackagesResponse?.data?.map(
                          (value: GzPackage, key: number) => (
                            <option
                              value={value._id}
                              className="text-body dark:text-bodydark"
                              key={key}
